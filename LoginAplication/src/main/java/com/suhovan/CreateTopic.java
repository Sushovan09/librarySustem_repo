...
public class CreateTopic extends HttpServlet{
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException,IOException {
		...
		try {
			...
			String query = String.format("SELECT  SubjectName FROM subject");
			...
			if(rs.next()) {
				...
				pw.println("<h2><b>Select Subject Name To Add Topic</b></h2>");
				pw.println("<form action=\"createTopicDatabase\" method =\"POST\">");
				pw.println("<select name=\"Department\" id=\"subject\">");	
				pw.println("<option value=\""+rs.getString(1)+"\">"+rs.getString(1)+"</option>");
				while(rs.next()) {
					pw.println("<option value=\""+rs.getString(1)+"\">"+rs.getString(1)+"</option>");
				}
				pw.println("<input type = \"text\" class =\"text\" name= \"TopicName\" placeholder = \"Enter New Topic Name\"><br><br>");
